//! Drive the built `git-comment` binary against a temp repo, exactly as
//! `git comment …` would.
...
const BIN: &str = env!("CARGO_BIN_EXE_git-comment");
...
fn add_with_path_and_lines_then_list_and_show() {
    ...
    assert!(
        out.contains("Test"),
        "list output should show author: {out}"
    );
    ...
    assert!(
        out.contains("author: Test <test@example.com>"),
        "show output: {out}"
    );
    assert!(out.contains("date:"), "show output: {out}");
}
...
fn add_without_path_attaches_to_head_commit() {
    ...
    let (out, err, ok) = run(path, None, &["add", "-m", "whole commit comment"]);
    assert!(ok, "add failed: {err}");
    let id = out.trim().to_owned();
    ...
    assert!(out.contains("whole commit comment"), "show output: {out}");
}
...
fn show_accepts_a_prefix_and_rejects_ambiguous_or_missing_ids() {
    ...
    // A single hex digit matches every comment in this repo (there's only
    // one), so pad the fixture with a second comment to exercise ambiguity.
...
#[test]
fn remove_deletes_a_comment_and_a_second_removal_fails() {
    let dir = tempfile::tempdir().unwrap();
    setup(dir.path());
    let path = dir.path();
    ...
    assert!(err.contains("no comment"), "stderr: {err}");
}
...
fn bare_invocation_lists_instead_of_erroring() {
    ...
    assert_eq!(out.trim(), "", "no comments yet: {out}");

    let (out, _, ok) = run(path, None, &["add", "-m", "a comment"]);
    assert!(ok);
    let id = out.trim().to_owned();
...
#[test]
fn edit_replaces_the_message_and_keeps_the_same_id() {
    let dir = tempfile::tempdir().unwrap();
    setup(dir.path());
    let path = dir.path();

    let (out, _, ok) = run(path, None, &["add", "-m", "first message"]);
    assert!(ok);
    let id = out.trim().to_owned();

    let (out, err, ok) = run(path, None, &["edit", &id, "-m", "second message"]);
    assert!(ok, "edit failed: {err}");
    assert_eq!(out.trim(), id, "edit reattaches the same identity oid");

    let (out, err, ok) = run(path, None, &["show", &id]);
    assert!(ok, "show failed: {err}");
    assert!(out.contains("second message"), "show output: {out}");
    assert!(!out.contains("first message"), "show output: {out}");
}
...
#[test]
fn log_prints_every_version_newest_first_with_author() {
    let dir = tempfile::tempdir().unwrap();
    setup(dir.path());
    let path = dir.path();
    ...
    assert!(lines[0].contains("v2"), "log output: {out}");
    assert!(lines[1].contains("v1"), "log output: {out}");
    for line in &lines {
        let mut fields = line.split_whitespace();
        let oid = fields.next().expect("oid field");
        assert_eq!(oid.len(), 40, "log line: {line}");
        assert!(line.contains("Test"), "log line should show author: {line}");
    }
}
...
fn show_rejects_reflog_syntax() {
    ...
    let (out, _, ok) = run(path, None, &["add", "-m", "comment"]);
    assert!(ok);
    let id = out.trim().to_owned();
...
#[test]
fn list_by_commit_includes_position_comments_anchored_there() {
    let dir = tempfile::tempdir().unwrap();
    setup(dir.path());
    let path = dir.path();
    ...
    assert!(
        ...
        "list HEAD should include the position comment anchored at HEAD: {out}"
    );
    assert!(
        out.contains(&commit_id[..8]),
        "list HEAD should include the commit comment: {out}"
    );
}
...
fn list_json_emits_one_object_per_line() {
    ...
    assert!(out.contains("\"author\":\"Test\""), "list --json: {out}");
    assert!(out.contains("\"time\":"), "list --json: {out}");
}
...
fn remove_accepts_multiple_ids_and_is_atomic_on_a_bad_one() {
    ...
    let (out, _, ok) = run(path, None, &["list"]);
    assert!(ok);
    assert_eq!(out.trim(), "", "both comments should be gone: {out}");
}

#[test]
fn attach_resolves_a_tree_ish_and_round_trips_the_oid() {
    let dir = tempfile::tempdir().unwrap();
    setup(dir.path());
    let path = dir.path();

    // Attach HEAD's own tree to the comment.
    ...
    let tree_out = String::from_utf8_lossy(&output.stdout).trim().to_owned();

    let (out, err, ok) = run(
        path,
        None,
        &["add", "-m", "with attachment", "--attach", "HEAD"],
    );
    assert!(ok, "add --attach failed: {err}");
    let id = out.trim().to_owned();

    let (out, err, ok) = run(path, None, &["show", &id]);
    assert!(ok, "show failed: {err}");
    assert!(
        out.contains(&format!("attachment: {tree_out}")),
        "show output: {out}"
    );
...
#[test]
fn attach_rejects_a_non_tree_ish() {
    let dir = tempfile::tempdir().unwrap();
    setup(dir.path());
    let path = dir.path();
    ...
    let (_, err, ok) = run(
        path,
        None,
        &["add", "-m", "bad attach", "--attach", "not-a-real-object"],
    );
    assert!(!ok, "attach with an unresolvable spec should fail");
    assert!(err.contains("cannot resolve"), "stderr: {err}");
}

#[test]
fn edit_keeps_existing_attachment_when_attach_not_given() {
    let dir = tempfile::tempdir().unwrap();
    setup(dir.path());
    let path = dir.path();

    let (out, err, ok) = run(path, None, &["add", "-m", "first", "--attach", "HEAD"]);
    assert!(ok, "add --attach failed: {err}");
    let id = out.trim().to_owned();

    let (out, err, ok) = run(path, None, &["show", &id, "--json"]);
    assert!(ok, "show --json failed: {err}");
    assert!(out.contains("\"attachment\":"), "show --json: {out}");

    let (_, err, ok) = run(path, None, &["edit", &id, "-m", "second"]);
    assert!(ok, "edit failed: {err}");
