...
//! paths `refs/comments/*` records.
//!
//! The lens caches nothing derived — no projection, no lens, no diagnostic
//! survives a comment mutation (`lens.lenses`) — but it must remember the
//! *buffer text* the client has sent, because the client owns the only copy
//! of a document's unsaved content: `textDocument/didChange` ships an edit,
//! never the file, and the disk still holds the old bytes. Holding the
//! latest buffer per open URI is what lets projection target the bytes the
//! user is actually looking at (`lens.working-tree`), and it is dropped the
//! moment the client closes the document.
...
impl Documents {
    ...
    /// these after a comment mutation.
    #[must_use]
...
/// under it — the key that matches an [`gix_anchor::Anchor`]'s own recorded
/// path.
///
