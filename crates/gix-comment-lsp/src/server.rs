...
//! the lens exactly: every operation it performs — reading `refs/comments/*`,
//! diffing against the working tree, writing a new commit — is blocking git
//! and filesystem work, so an async runtime would only wrap blocking calls
//! in `spawn_blocking` for no gain.
#![expect(
...
/// # Errors
///
/// [`crate::Error::Io`] if the JSON-RPC transport fails (a broken pipe, a
/// malformed frame) or the initialize handshake does not complete.
pub fn serve(repo: gix::Repository) -> crate::error::Result<()> {
...
/// The running dispatch loop: owns the connection and the lens.
struct ServerLoop {
...
impl ServerLoop {
    fn run(&mut self) -> std::io::Result<()> {
        // `recv()` yields until the client closes the connection; a
    ...
    where
        ...
        F: FnOnce(&Lens, R::Params) -> crate::error::Result<R::Result>,
    ...
    /// with its result value, open a compose/reply template if it named
    /// one, and republish diagnostics when a mutation invalidated them.
...
fn log(message: &str) {
    eprintln!("gix-comment-lsp: {message}");
}
