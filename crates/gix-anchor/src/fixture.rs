...
//! this crate's test suites drive it with.
//!
//! Compiled only under `cfg(test)`. When a second crate needs these
//! helpers, they move to a shared test-support dev-dependency crate the
//! workspace test strategy calls for; extracting them now would create a
//! crate no second consumer exists for yet.
